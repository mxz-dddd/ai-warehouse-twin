...
校准与优化建议闭环是 planned / not yet implemented 的后续产品能力。当前基线只提供确定性仿真 artifact、A/B comparison artifact、customer Markdown report、input validation baseline 以及 golden/smoke/CI；误差区间、可信度等级 A–D、历史数据校准和闭环优化建议将在 R5/R7 阶段实现。
...
AI Warehouse Twin is proprietary software. All rights reserved.

Public repository visibility does not grant an open-source license or permission to use, copy, modify, distribute, deploy, host, commercialize, or reuse the software, datasets, schemas, generated contracts, artifacts, reports, documentation, or related materials.

本项目是面向真实仓库客户的创业产品，不是开源项目。仓库可见性不等于授予开源许可。

See `LICENSE` and `NOTICE`.

## Contract governance

Contracts and artifact golden files are treated as product handoff boundaries. See `docs/architecture/contracts-v1-freeze.md`.

Changing contracts requires a dedicated `CONTRACT-` task, version review, codegen / drift check, explicit artifact-golden review when applicable, and product/core + visualization/handoff review.
...
上图是产品路线目标，不表示当前已经实现可信度分级或闭环优化建议。
...
- 当前仓库尚未实现 calibration / optimization product services；任何校准可信度、误差区间、可信度等级 A–D 或闭环优化建议都必须标为 planned / not yet implemented，直到对应 R5/R7 任务具备测试、artifact 与 CI guard。
...
Calibration and optimization are planned product capabilities, not yet implemented in the current MVP baseline. Current outputs are deterministic simulation artifacts, A/B comparison artifacts, and customer Markdown reports. Confidence grading, error intervals, historical-data calibration, and closed-loop optimization recommendations will be implemented in later R5/R7 phases.
