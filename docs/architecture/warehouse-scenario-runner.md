...
Status: planned / not yet implemented for calibration and optimization. 当前 runner 不输出误差区间、可信度等级 A–D、历史数据校准结果或闭环优化建议；这些能力必须等待 R5/R7 独立任务卡。
...
8. 不得把 optimization / calibration / confidence grading 写成已实现，除非 R5/R7 对应任务已经落地并具备测试、artifact 与 CI guard。
