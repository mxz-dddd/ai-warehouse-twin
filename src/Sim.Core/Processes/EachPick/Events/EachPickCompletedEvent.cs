...
public sealed class EachPickCompletedEvent : ISimEvent
{
    private readonly EachPickSimulationState _state;

    public EachPickCompletedEvent(
        ...
        long occursAtMs)
    {
        ...
        {
            throw new DomainRuleViolationException("EachPickCompletedEvent OrderId cannot be empty.");
        }

        if (occursAtMs < 0)
        {
            throw new DomainRuleViolationException(
                $"EachPickCompletedEvent OccursAtMs cannot be negative. OccursAtMs: {occursAtMs}.");
        }
    ...
    public string EventId => $"each_pick.completed.{OrderId}";

    public long OccursAtMs { get; }

    public string EventType => "EachPickCompleted";
    ...
    public void Execute(SimulationContext context)
    {
        ...
        var candidate = _state.Inventory.Values
            .Where(item =>
                ...
                item.Status == InventoryStatus.Picking &&
        ...
        {
            throw new DomainRuleViolationException(
                $"Each-pick order has no PICKING inventory to complete. OrderId: {order.OrderId}, SkuId: {order.SkuId}, Quantity: {order.Quantity}.");
        }

        InventoryStateMachine.EnsureCanTransition(candidate.Status, InventoryStatus.Picked);
