...
namespace AIWarehouseTwin.UI
{
    public static class KpiSummaryFormatter
    {
        ...
        public static KpiSummaryRow[] FormatStructured(RunArtifactDto artifact)
        {
            ...
            var rows = new List<KpiSummaryRow>
            {
                ...
                new KpiSummaryRow(
                    "Overview",
                    "Duration",
                    $"{kpi.total_duration_ms.ToString(CultureInfo.InvariantCulture)} ms"),
                new KpiSummaryRow(
                    "Overview",
                ...
                new KpiSummaryRow("Throughput", "Outbound/hour", FormatRate(kpi.outbound_order_throughput_per_hour)),
                new KpiSummaryRow("Throughput", "Each-pick/hour", FormatRate(kpi.each_pick_order_throughput_per_hour)),
                new KpiSummaryRow("Throughput", "Total/hour", FormatRate(kpi.total_work_item_throughput_per_hour)),
            ...
            foreach (var bottleneck in OrderedBottlenecks(kpi.bottlenecks))
            {
                rows.Add(new KpiSummaryRow(
                    "Bottlenecks",
                    $"#{bottleneck.rank.ToString(CultureInfo.InvariantCulture)} {bottleneck.resource_id} ({bottleneck.resource_type})",
        ...
        {
            return value.HasValue ? FormatMilliseconds(value.Value) : "N/A";
        }
        ...
        {
            return entries == null
                ? Enumerable.Empty<KeyValuePair<string, double>>()
            ...
            IEnumerable<RunArtifactBottleneckDto> bottlenecks)
        {
            return bottlenecks == null
                ? Enumerable.Empty<RunArtifactBottleneckDto>()
                : bottlenecks
