...
namespace AIWarehouseTwin.Tests
{
    public sealed class KpiHudPanelTests
    {
        ...
        public void RefreshUi_replaces_existing_rows_without_accumulating_labels()
        {
            ...
            KpiHudPanel.RefreshUi(first, rootElement);
            KpiHudPanel.RefreshUi(second, rootElement);

            var labels = rootElement.Query<Label>().ToList().Select(label => label.text).ToArray();
